...
현재 카테고리 정보를 가지고 있다.
현재 카테고리 하위의 "카테고리 번 호"를 목록으로 가지고 있다.
...
public:
	...
	void SetCategoryFirst();							// 하위 카테고리 목록의 첫 번째 항 록을 가리키도록 설정한다.
...
public:
	...
	int EventFlag;										// 3. 이벤트 카테고리 여부 (199:이 벤트, 200:일반)	
	int OpenFlag;										// 4. 공개 여부 (201:공개, 202: 비 공개)
