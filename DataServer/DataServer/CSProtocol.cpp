...
void FriendMemoSend(FHP_FRIEND_MEMO_SEND* lpMsg, int index)
{
	...
	gQueryManager.BindParameterAsBinary(2, lpMsg->Equipment, sizeof(lpMsg->Equipment));
	...
	if (pMsg.Result == 1)
	{
		...
		if (gCharacterManager.GetCharacterInfo(&CharacterInfo, lpMsg->ToName) != 0)
		{
			...
			{
				CSDataSend(lpServerManager->m_index, (BYTE*)&pMsg, sizeof(pMsg));
			}
			gQueryManager.Close();
		}
	}
...
void FriendMemoReadReq(FHP_FRIEND_MEMO_RECV_REQ* lpMsg, int index)
{
	...
	gQueryManager.GetAsBinary("Photo", (BYTE*)pMsg.Equipment, sizeof(pMsg.Equipment));
