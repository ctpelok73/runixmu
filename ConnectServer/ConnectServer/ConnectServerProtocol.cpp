			...
			if (connection_check_update(index) == true)
			{
				...
				for (int version = ClientVersion; version <= ServerVersion; version++)
				{
					...
					while (offset < totalSize)
					{
						...
						memset(send, 0, sizeof(send)); // Aseg�rate de inicializar el buffer
