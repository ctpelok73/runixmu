...
Если удобнее проверять готовый бинарник после каждого шага, используй self-hosted CI-пайплайн
из `.github/workflows/gameserver-build.yml` в ручном режиме (`workflow_dispatch`).
Подробный сценарий запуска см. в `GameServer_CI_DOWNLOAD_FLOW.md`.
