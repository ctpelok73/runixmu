...
void ProtocolCore(BYTE head, BYTE* lpMsg, int size, int aIndex, int encrypt, int serial) // OK
{
...
#if (CB_GETMIXRATE)
	case 0x88:
	if (gServerInfo.m_UseZenRateReal == 1)
	{
	...
	case 0x1E:
		...
		break;
	case 0x20:
		CGCharacterOffRecv((PMSG_CHARACTER_OFF_RECV*)lpMsg, aIndex);
		break;
	case 0x22:
	...
	case 0x24:
		...
		break;
	case 0x25:
		gItemManager.CGItemPostChatRecv((PMSG_ITEM_BUY_RECV*)lpMsg, aIndex);
		break;
	case 0x26:
	...
	case 0x35:
		gItemManager.CGInventoryBuyRecv((PMSG_ITEM_BUY_RECV*)lpMsg, aIndex);
		break;
	...
	case 0x4F:
		...
		case 0x03:
			GoblinMixExpansion->ReceiveMixInfo(aIndex, lpMsg);
			break;
		case 0x06:
			GoblinMixExpansion->CreateMix(aIndex, lpMsg);
			break;
#endif // CHAOS_MACHINE_EXTENSION
		case 0x8:
#ifdef INVENTORY_HOLY_JOH
			GMHolyItem->GCInventoryRemove(aIndex, (PMSG_INVENTORY_HOLY_RECV*)lpMsg);
#endif // INVENTORY_HOLY_JOH
			break;
		case 0x9:
#ifdef INVENTORY_HOLY_JOH
			GMHolyItem->GCInventoryAddItem(aIndex, (PMSG_INVENTORY_HOLY_RECV*)lpMsg);
#endif // INVENTORY_HOLY_JOH
			break;
		}
		break;
	...
	case 0x66:
		...
		break;
	case 0x71:
		CGReqtimeState((PMSG_DATA_PACKAGE*)lpMsg, aIndex);
		break;
	case 0x81:
	...
	case 0xD3:
		...
		case 0x01:
		{
			if (lpObj->ClickClientSend + 1000 > GetTickCount())
			{
				return;
			}
	...
	case 0xF3:
		...
		case 0x06:
			...
			break;
		case 0x08:
			CGReloadInterfaceMain((PMSG_LEVEL_UP_POINT_RECV*)lpMsg, aIndex);
			break;
		case 0x12:
		...
		case 0xED:
			...
			break;
		case 0xF0:
			gShopbuyvip->ReceiveBuyAccountVip((BUYPREMIUM_REQ*)lpMsg, aIndex);
			break;
		case 0xF2:
...
void CGChatRecv(PMSG_CHAT_RECV* lpMsg, int aIndex) // OK
{
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(269), lpObj->ChatLimitTime);
		return;
	}
	...
	{
		if (GMAntiSpam->InvokeMessage(lpObj->Account, lpMsg->message) == false)
		{
...
void CGChatWhisperRecv(PMSG_CHAT_WHISPER_RECV* lpMsg, int aIndex) // OK
{
	...
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(270));
		return;
	}
	...
	if (gParty.IsLeader(lpTarget->PartyNumber, lpTarget->Index) != 0)
	{
		if (gParty.PartyPasswordCheck(lpTarget->PartyNumber, std::string(lpMsg->message)) != 0)
		{
			lpObj->CurAutoJoinParty = lpTarget->PartyNumber;
			lpObj->CurAutoJoinPartyPassword.assign(lpMsg->message);
		}
		else
		{
...
void CGCharacterOffRecv(PMSG_CHARACTER_OFF_RECV* lpMsg, int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
...
void CGPetItemInfoRecv(PMSG_PET_ITEM_INFO_RECV* lpMsg, int aIndex) // OK
{
	...
	else if (lpMsg->flag == 6) // shop normal
	{
		if (SHOP_INVENTORY_RANGE(lpMsg->slot) == 0)
		{
			return;
		}

		if (lpObj->TargetShopNumber != -1)
		{
			CItem* ShopInventory = gShopManager.GetInventory(lpObj->TargetShopNumber);

			if (ShopInventory)
			{
				GCPetItemInfoSend(aIndex, lpMsg->type, lpMsg->flag, lpMsg->slot, ShopInventory[lpMsg->slot].m_PetItemLevel, ShopInventory[lpMsg->slot].m_PetItemExp, (BYTE)ShopInventory[lpMsg->slot].m_Durability);
			}
		}
...
void CGConnectAccountRecv(PMSG_CONNECT_ACCOUNT_RECV* lpMsg, int aIndex) // OK
{
	...
	PacketArgumentDecrypt(HardwareId, lpMsg->ComputerHardwareId, (sizeof(HardwareId) - 1));

	if (gServerInfo.m_MaxHWIDConnection > 0)
	{
		if (gmHardwareId.CheckHardwareId(HardwareId))
		{
			strcpy(lpObj->m_HardwareId, HardwareId);

			gmHardwareId.InsertHardwareId(lpObj->m_HardwareId);
		}
		else
		{
			CloseClient(aIndex);
...
void CGCharacterCreateRecv(PMSG_CHARACTER_CREATE_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpMsg->Class != DB_CLASS_DW
		&& lpMsg->Class != DB_CLASS_DK
		&& lpMsg->Class != DB_CLASS_FE
		&& lpMsg->Class != DB_CLASS_MG
		&& lpMsg->Class != DB_CLASS_DL
		&& lpMsg->Class != DB_CLASS_SU
		&& lpMsg->Class != DB_CLASS_RF
		&& lpMsg->Class != DB_CLASS_GL
		&& lpMsg->Class != DB_CLASS_RW
		&& lpMsg->Class != DB_CLASS_SL
		&& lpMsg->Class != DB_CLASS_GC
		&& lpMsg->Class != DB_CLASS_KUM
		&& lpMsg->Class != DB_CLASS_LEM
		&& lpMsg->Class != DB_CLASS_KIN
		)
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	...
	if (lpMsg->Class == DB_CLASS_GL && (lpObj->ClassCode & 16) == 0)
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
...
void CGReloadInterfaceMain(PMSG_LEVEL_UP_POINT_RECV* lpMsg, int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	gItemManager.GCItemListSend(aIndex, true);

#if(GAMESERVER_UPDATE>=401)
	gMasterSkillTree.GCMasterSkillListSend(aIndex);
#endif

	gQuest.GCQuestInfoSend(lpObj->Index);

#if(GAMESERVER_UPDATE>=501)
	gQuestWorld.GCQuestWorldInterfaceListSend(lpObj);

	gQuestWorld.GCQuestWorldButtonSend(lpObj, 0, 1, 0);

	gGensSystem.CalcGensMemberInfo(lpObj);
#endif
}
...
void GCEventStateSend(int aIndex, BYTE state, BYTE _event) // OK
{
...
void GCEventStateSendToAll(int map, BYTE state, BYTE _event) // OK
{
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		if (gObjIsConnectedGP(n) != 0)
		{
			if (map == gObj[n].Map)
			{
				GCEventStateSend(n, state, _event);
			}
		}
	}
}
...
void GCCoinSend(int aIndex, DWORD coin1, DWORD coin2, DWORD coin3)
{
	PMSG_COIN_SEND pMsg;
...
void GCLifeUpdateSend(LPOBJ pPlayer) // OK
{
#if(GAMESERVER_UPDATE>=603)
	if (pPlayer->Type != OBJECT_MONSTER && pPlayer->Type != OBJECT_USER)
	{
		return;
	}

	PMSG_LIFE_UPDATE_SEND player_body;

	player_body.header.set(0xEC, 0x10, sizeof(player_body));

	player_body.SetIndex(pPlayer->Index);

	player_body.MaxHP = (pPlayer->MaxLife + pPlayer->AddLife);
	player_body.CurHP = pPlayer->Life;
	...
	MsgSendV2(pPlayer, (BYTE*)&player_body, player_body.header.size);
...
void GCPKLevelSend(int aIndex, int PKLevel, DWORD PenaltyTime) // OK
{
	...
	{
		pMsg.PenaltyTime = PenaltyTime;
	}
	else
	{
		pMsg.PenaltyTime = 0xFFFFFFFF;
	}
...
void GCNewCharacterCalcSend(LPOBJ lpObj) // OK
{
	...
	pMsg.PowerATTK = lpObj->PowerATTK;
...
{
#if(GAMESERVER_TYPE==1)
	PMSG_INFO_STATUE_AND_GATE pMsg;

	pMsg.h.set(0xB2, 0x0A, sizeof(pMsg));

	pMsg.iNpcUpMaxHP1[0] = g_iNpcUpMaxHP_CGATE[CS_MAX_UPGRADE][0];
	pMsg.iNpcUpMaxHP2[0] = g_iNpcUpMaxHP_CSTATUE[CS_MAX_UPGRADE][0];
	pMsg.iNpcUpRecLevel[0] = g_iNpcUpRgLevel_CSTATUE[CS_MAX_UPGRADE][0];

	for (size_t j = 0; j <= CS_MAX_UPGRADE; j++)
	{
		pMsg.iNpcDefense1[j] = g_iNpcDefense_CGATE[j];
		pMsg.iNpcDefense2[j] = g_iNpcDefense_CSTATUE[j];

		if (j < CS_MAX_UPGRADE)
		{
			pMsg.iNpcUpMaxHP1[j + 1] = g_iNpcUpMaxHP_CGATE[j][0];
			pMsg.iNpcUpMaxHP2[j + 1] = g_iNpcUpMaxHP_CSTATUE[j][0];
			pMsg.iNpcUpRecLevel[j + 1] = g_iNpcUpRgLevel_CSTATUE[j][0];
		}
	}
...
{
#if(GAMESERVER_TYPE==1)
	if (lpMsg != NULL && strcmp(gObj[iIndex].GuildName, "") != 0)
	{
		if ((gCastleSiege.CheckGuildOwnCastle(gObj[iIndex].GuildName) == FALSE) || (gObj[iIndex].GuildStatus != G_MASTER))
		{
